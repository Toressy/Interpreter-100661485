		...
		while self.current_char != None:
			if self.current_char in ' \t':
				...
				if self.current_char == '^':  
					...
					return [], IllegalCharError(pos_start, self.pos, "'^' should be '^^'")
			elif self.current_char == "!":
...
class Parser:
	...
	def statement(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', 'set', 'check', 'walk', 'keep', 'build', int, float, identifier, '+', '-' or '('"
            ))
	...
	def expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'set', 'check', 'walk', 'keep', 'build', int, float, identifier, '+', '-' or '('"
			))
	...
	def atom(self):
		...
		elif tok.matches(TT_KEYWORD, 'walk'):
		...
		return res.failure(InvalidSyntaxError(
			...
			"Expected int, float, identifier, '+', '-', '(', '[', 'check', 'walk', 'keep', 'build"
		))
	...
	def list_expr(self):
		...
		if self.current_tok.type == TT_RSQUARE:
			...
			if res.error:
				return res.failure(InvalidSyntaxError(
					self.current_tok.pos_start, self.current_tok.pos_end,
					"Expected ']', 'set', 'check', 'walk', 'keep', 'build', int, float, identifier, '+', '-', '(', '[' or 'not'"
					))
	...
	def if_expr(self):
		res = ParseResult()
		all_cases = res.register(self.if_expr_cases('check'))
		if res.error: return res
		cases, else_case = all_cases
		return res.success(IfNode(cases, else_case))
	
	def if_expr_b(self):
		return self.if_expr_cases('orif')
	...
	def if_expr_b_or_c(self):
		...
		if self.current_tok.matches(TT_KEYWORD, 'orif'):
	...
	def for_expr(self):
		...
		if not self.current_tok.matches(TT_KEYWORD, 'walk'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'walk'"
			))
		...
		if not self.current_tok.matches(TT_KEYWORD, 'thru'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'thru'"
			))
	...
	def while_expr(self):
		res = ParseResult()
		if not self.current_tok.matches(TT_KEYWORD, 'keep'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'keep'"
			))
...
class Number(Value):
	...
	def get_comparison_eq(self, other):
		if isinstance(other, Number):
			...
			return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot compare a Number ({self.value}) with a {type(other).__name__} ({other.value}) using '<'",
                self.context
			)
...
class Boolean(Value):  
    ...
    def dived_by(self, other):
        if isinstance(other, Boolean):
            ...
            return None, RTError(
            self.pos_start, other.pos_end,
            f"Cannot divide a Boolean ({self.value}) by a {type(other).__name__} ({other.value})",
            self.context
        )
    ...
    def powed_by(self, other):
        if isinstance(other, Boolean):
            ...
            return None, RTError(
            self.pos_start, other.pos_end,
            f"Cannot raise a Boolean ({self.value}) to a {type(other).__name__} ({other.value})",
            self.context
        )
    ...
    def get_comparison_eq(self, other):
        if isinstance(other, Boolean):
            ...
            return None, RTError(
				self.pos_start, other.pos_end,
				f"Cannot compare a Boolean ({self.value}) with a {type(other).__name__} ({other.value}) using '=='",
				self.context
			)
    ...
    def anded_by(self, other):
        if isinstance(other, Boolean):
            ...
            return None, RTError(
				self.pos_start, other.pos_end,
				f"Cannot perform 'and' between a Boolean ({self.value}) and a {type(other).__name__} ({other.value})",
				self.context
			)
    ...
    def ored_by(self, other):
        if isinstance(other, Boolean):
            ...
            return None, RTError(
				self.pos_start, other.pos_end,
				f"Cannot perform 'or' between a Boolean ({self.value}) and a {type(other).__name__} ({other.value})",
				self.context
			)
...
class String(Value):
	...
	def added_to(self, other):
		if isinstance(other, String):
			...
			return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot add a {type(other).__name__} ({other.value}) to a String (\"{self.value}\")",
                self.context
            )
	...
	def subbed_by(self, other):
		return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot substract a String (\"{self.value}\") by a {type(other).__name__} ({other.value})",
                self.context
            )
	...
	def multed_by(self, other):
		if isinstance(other, Number):
			...
			return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot multiply a String (\"{self.value}\") by a {type(other).__name__} ({other.value})",
                self.context
            )
	...
	def dived_by(self, other):
		return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot divide a String (\"{self.value}\") by a {type(other).__name__} ({other.value})",
                self.context
            )
	...
	def get_comparison_eq(self, other):
		if isinstance(other, String):
			...
			return None, RTError(
                self.pos_start, other.pos_end,
                f"Cannot compare a String (\"{self.value}\") with a {type(other).__name__} ({other.value}) using '=='",
                self.context
            )
